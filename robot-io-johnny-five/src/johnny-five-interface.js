...
export default createJFiveInterface

function createJFiveInterface(argv = {}) {
    const j5 = {
        onDataCallback(data) {
    ...
    createBoard(argv.serialPort).then(board => {
        ...
        board.on("ready", _ => {
            ...
            imu.on("data", _ => {
                ...
                let timeStamp = (millis - startMillis) / 1000

                //WAIT (non blocking) at least 50ms delta time
                if (deltaMillis > 50) {
                    const newData = {
                        timeStamp,
                        data: [
