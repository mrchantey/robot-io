...
namespace Ahoy.Navigation
{
    ...
    public class PathGenerator : ComponentSystem
    {
        ...
        public AhoyEvent<PathInfo[]> onPathsGenerated;
        List<PathGenerateInfo> pathsToGenerate;
        NodeGroup group;

        protected override void OnCreate()
        {
            manager = World.Active.EntityManager;
            pathsToGenerate = new List<PathGenerateInfo>();

            onPathsGenerated = new AhoyEvent<PathInfo[]>();
            groupQuery = manager.CreateEntityQuery(new ComponentType[] { typeof(NodeGroup) });
        }

        public void SetNodeGroup(NodeGroup _group)
        {
            group = _group;
        }

        public void AddPathToGenerate(PathGenerateInfo generateInfo)
        {
            pathsToGenerate.TryAdd(generateInfo);
            var pathInfo = GeneratePath(group, generateInfo.start.position, generateInfo.finish.position);
            generateInfo.onPathGenerated(pathInfo);
            onPathsGenerated.Invoke(new PathInfo[] { pathInfo });
        }
        ...
        public bool GeneratePaths()
        {
            if (pathsToGenerate.Count == 0)
                return false;
            var infos = new PathInfo[pathsToGenerate.Count];
            pathsToGenerate.ForEach((gi, i) =>
            {
                var info = GeneratePath(group, gi.start.position, gi.finish.position);
                infos[i] = info;
                gi.onPathGenerated(info);
            });
            onPathsGenerated.Invoke(infos);
            return true;
        }

        PathInfo GeneratePath(NodeGroup group, Vector3 startPosition, Vector3 endPosition)
        {
